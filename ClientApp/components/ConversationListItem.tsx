...
class ConversationListItem extends React.Component<IConversationProps, { }> {
    public render() {
        return <Link to={'/' + this.props.conversation.id} style={{height: "200px"}} className={ "list-group-item" + (this.props.conversation.active ? " active" : "") }>
            ...
            <p className="list-group-item-text">{ this.props.conversation.participants.map(participant => 
                '@' + participant.user.username).slice(0,3).join() }</p>
        </Link>;
    }
}
